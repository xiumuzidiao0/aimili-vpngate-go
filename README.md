...
| **多出口并发与端口矩阵** | 单端口、单出口单连 | **多出口同时并发在线 (`tun0`, `tun1`...)，M:N 端口分流调度 (轮询/随机/定时)** |
| **动态自适应隧道池** | 无 | **自动按国家/网络类型维持 Top N 最优节点在线，后台自动无损平滑轮换** |
| **AI 与流媒体解锁检测** | 无 | **内置 ChatGPT、Claude、Netflix 原生流媒体及 Google 解锁状态探测** |
| **节点长期历史信誉库** | 仅单次测速 | **本地持久化记录节点真实稳定性、在线时长与断线率，智能避开频繁闪退节点** |
| **深度隐私与指纹脱敏** | 基础转发 | **深度清洗 `Via`、`X-Forwarded-For`、`CF-Connecting-IP` 等所有代理标记头** |
| **Telegram 交互与告警** | 纯控制台输出 | **TG Bot 自动推送故障转移/断线告警，支持 `/status`、`/rotate` 远程控制指令** |
